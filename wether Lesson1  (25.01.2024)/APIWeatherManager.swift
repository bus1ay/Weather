...
enum ForcecastType: FinalURLPoint {
    case Current(apiKey: String, coordinates: Coordinates)
    
    var baseURL: URL {
        return  URL(string: "https://api.forcast.io")!
    }
    var path: String {
        switch self {
...
final class APIWeatherManager: APIManager {
    ...
    func fetchCurrentWeatherWith(coordinates: Coordinates, completionHandler: @escaping (APIResult<CurrentWeather>) -> Void) {
        let request = ForcecastType.Current(apiKey: self.apiKey, coordinates: coordinates).request
